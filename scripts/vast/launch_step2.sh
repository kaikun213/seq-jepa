#!/usr/bin/env bash
# Launch Step 2 remote experiments on Vast.ai
# Runs both Exp A (EMA+Rate) and Exp B (Teacherless) sequentially
set -euo pipefail
...
LABEL="${VAST_LABEL:-seqjepa-step2}"
...
echo "========================================="
echo "Launching Step 2 Remote Experiments"
echo "Exp A: EMA + Rate (30 epochs)"
echo "Exp B: Teacherless + Rate (30 epochs)"
...
CREATE_JSON=$(vastai create instance "$OFFER_ID" \
  ...
  --label "$LABEL" \
  --ssh --direct \
  --onstart "$REPO_ROOT/scripts/vast/onstart_step2.sh" \
  --env "$ENV_STRING" \
...
POLL_SECS="${VAST_POLL_SECS:-120}"
DONE_PATH="${VAST_DONE_PATH:-/workspace/seq-jepa-streaming/runs/vast_step2.done}"
FAIL_PATH="${VAST_FAIL_PATH:-/workspace/seq-jepa-streaming/runs/vast_step2.failed}"
...
PY
    ...
    if [[ -n "$SSH_HOST" && -n "$SSH_PORT" && "$SSH_HOST" != "$SSH_PORT" ]]; then
      if ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$SSH_PORT" "root@${SSH_HOST}" "test -f '$DONE_PATH'" 2>/dev/null; then
        echo "Run completed successfully."
        break
      fi
      if ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -p "$SSH_PORT" "root@${SSH_HOST}" "test -f '$FAIL_PATH'" 2>/dev/null; then
        echo "Run failed."
        break
      fi
    fi
